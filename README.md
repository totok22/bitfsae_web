...
- `main`：新服务器（腾讯云）与新域名环境对应分支。
- `legacy-aliyun-xin`：旧服务器（阿里云）与 `.xin` 域名环境对应分支。
- 当前仓库若用于旧服务器维护，请在 `legacy-aliyun-xin` 分支上提交，避免把旧环境配置直接提交到 `main`。
- 如需从 `main` 同步公共文档或脚本，建议先在 `legacy-aliyun-xin` 拉取后再按需合并，避免覆盖旧服务器专用配置。
